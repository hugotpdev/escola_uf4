        ...
        subjects.forEach( subject => {
            ...
            contentTable += `
                <div class="tr">
                    ...
                    <div class="td">
                        <button class="btn--action update-btn" id="${subject.id}">
...
function createModal(id){
    ...
    Common.getEnrollmentsOfSubject(id, enrollments => {
        ...
        enrollments.forEach( enroll => {
            content += `
            <li>
                ${enroll.student.user.last_name}, ${enroll.student.user.first_name}
            </li>
        `;
        });
        ...
        fondo.innerHTML = `
            <div class="modal">
                <h1>Asignaturas</h1>
                <ul>
                    ${content || "No tiene ninguna inscripción"}
                </ul>
                <div>
...
function createHTML(){
    Common.getListSubject(subjects => {
        ...
        allBtnUpdate.forEach( btn => {
            btn.addEventListener('click', e => {
                window.location.href = `./updateSubject.html?id=${e.target.closest('.update-btn').id}`;
            })
        });   
    });
}