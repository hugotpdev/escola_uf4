...
document.addEventListener('DOMContentLoaded', () => {
    ...
    Common.getTeacher(id, teacher => {
        ...
        Common.addInfoToInputs(teacher.user);

        const option = document.createElement('OPTION');
        option.textContent = teacher.department.name;
        option.value = teacher.department.id;
        option.selected = true;
        departmentId.appendChild(option);     
    
        Common.getListDepartment(departments => {
            departments = departments.filter(department => department.name != teacher.department.name);
...
btnCancel.addEventListener('click', () => window.location.href = './teacher.html');
